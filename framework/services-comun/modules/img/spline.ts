...
export class Spline {
    /* STATIC */
    private static solve(A: Float64Array[], ks: Float64Array): Float64Array {
        const m = A.length;
        let h = 0;
        let k = 0;
        while (h < m && k <= m) {
    ...
    private static zerosMat(r: number, c: number): Float64Array[] {
        const salida = [];
        for (let i = 0; i < r; i++) {
            salida.push(new Float64Array(c));
        }
        return salida;
    }
    ...
    constructor(private readonly xs: number[], private readonly ys: number[]) {
        this.ks = this.getNaturalKs(new Float64Array(this.xs.length));
    }

    private getNaturalKs(ks: Float64Array): Float64Array {
        const n: number = this.xs.length - 1;
        const A: Float64Array[] = Spline.zerosMat(n + 1, n + 2);

...
export function valoresValidos(listado: number[]): number[] {
    if (listado.length<3) {
        return [];
    }

    const minimo = listado.reduce((minimo: number, actual: number)=>actual<minimo ? actual : minimo);
    const maximo = listado.reduce((maximo: number, actual: number)=>actual>maximo ? actual : maximo);

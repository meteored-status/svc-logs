...
export type TElasticNode = "http" | "master" | "data" | "other";

export class Elastic extends BackendRequest {
    /* STATIC */
    ...
    public static async search(token: string, interval?: string, clusters?: string[], nodes?: string[], range?: {start: number; end: number;}): Promise<RequestResponse<ISearchOUT>> {
        let url = `${this.SERVICIO}/backend/elastic-monitor/search`;
    ...
    public static async listClusters(token: string): Promise<RequestResponse<IClusterListOUT>> {
        const url = `${this.SERVICIO}/backend/elastic-monitor/cluster/list`;
        return this.get<IClusterListOUT>(url, {auth: token});
    }

    @logRejection(true)
    public static async searchNodes(token: string, clusters?: string[], types?: TElasticNode[]): Promise<RequestResponse<INodesOUT>> {
        let url = `${this.SERVICIO}/backend/elastic-monitor/nodes`;

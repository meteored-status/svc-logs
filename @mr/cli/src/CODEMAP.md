...
> **El código vivía en `@mr/cli/src/mrpack/` hasta el 2026-09-04.** El nivel sobraba desde que
> `mrlang` se mudó a `@mr/core-i18n` y este `src/` quedó con un solo CLI dentro. Las rutas de
> este documento son ya las nuevas; si algo enlaza a la vieja, está caducado.
...
│                                    (la clase base Modulo vive en @mr/core-cli/modulo)
│
├── modulos/
...
### `Modulo` — la clase base, ahora en `@mr/core-cli/modulo`

Salió de aquí el 2026-09-04, al separar `mrlang` en su propio workspace: la compartían las dos CLI.
Su firma y su comportamiento están en
[`@mr/core-cli/CODEMAP.md`](../../core/cli/CODEMAP.md). Los 8 módulos de `modulos/` la extienden
igual que antes, solo cambia de dónde se importa:
...
### `Colors` — ahora en `@mr/core-cli`

Los dos ficheros de colores salieron de aquí el 2026-09-04: las constantes ANSI están en
`@mr/core-cli/colors/base` y la paleta cíclica de 21 combinaciones —la que da a cada workspace su
color en `mrpack devel`— en `@mr/core-cli/colors`. Firmas en
[`@mr/core-cli/CODEMAP.md`](../../core/cli/CODEMAP.md).

`utiles/tty.ts` es el único consumidor de la base que queda en `mrpack`; el resto usa la cíclica.

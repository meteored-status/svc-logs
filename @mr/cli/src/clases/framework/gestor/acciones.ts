 ...
 * Hash: 7456db31f67e8cb14748ea1ac6149dac
 * Versión: 2026.9.7+1-bixus
 * Anterior: 2026.7.14+1-josantoniojimnez
 * Proyecto: https://github.com/meteored-status/svc-status.git
 */

import {mkdir, readFileString, safeWrite, unlink} from "@mr/core-cli/fs";
...
import {add, checkMrlang, getAutor, getClienteHash, leerDepsMrFramework, limpiarDevDepsConsumidores, recompilarCliente} from "../cliente";
...
export async function ejecutarAcciones(basedir: string, infos: IPaqueteGestion[], accionesArr: Accion[], config: IEjecutarAccionesConfig = {}): Promise<boolean> {
    ...
    // Y `mrlang` si lo que se actualizó fue el suyo. Se comprueba siempre en vez de mirar qué
    // paquetes venían en la tanda: el check es un hash de dos ficheros y no cuesta nada, y así
    // cubre también el caso de haberlo dejado a medias en una pasada anterior.
    await checkMrlang(basedir);

...
async function checkCliente(basedir: string): Promise<void> {
    ...
    // `mrlang` vive en su propio workspace desde que se separaron las dos CLI, así que su bundle
    // ya no se rehace con el de `mrpack`. Va después del cliente y sin reinicio: no es el proceso
...
async function initBase(basedir: string): Promise<IWorkspaces[]> {
    ...
    paquete.bin ??= {};
    paquete.bin["mrlang"] = "@mr/core/i18n/bin/mrlang.js";
